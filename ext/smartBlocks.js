...
(() => {
  ...
  roam42.smartBlocks.initialize = async () => {
    ...
    if (customTrigger != null && customTrigger.length > 0) {
      ...
      if (newTrigger.length > 0) smartBlockTrigger = customTrigger;
    }
    ...
    roam42.smartBlocks.UserDefinedWorkflowsList = async (
      ...
      let sbList = await roam42.common.getBlocksReferringToThisPage(
        "42SmartBlock"
      );
      let results = [];
      for (var sb of sbList) {
        try {
          ...
          if (sKey.trim() != "") {
            if (
              hideWorkflow == false ||
              (hideWorkflow == true && bHide == false)
            )
              results.push({
                key: sKey.trim(),
                value: sb[0].uid,
                processor: "blocks",
                fullCommand: sb[0].string
              });
          }
        } catch (e) {}
      }
    ...
    roam42.smartBlocks.activeWorkflow.outputAdditionalBlock = async (
      blockText,
      reprocessBlock
    ) => {
      // blockText - text that should be writen out to a block
      // reprocess - will run the block through the processor one more time
      await roam42.smartBlocks.activeWorkflow.arrayToWrite.push({
        text: blockText,
        reprocess: reprocessBlock
      });
    };

    const insertSnippetIntoBlock = async (
      textToInsert,
      removeIfCursor = true,
      startPosistionOffset = 2,
      caretRepositionDelay = 100
    ) => {
      setTimeout(async () => {
        ...
        var startPos = -1;
        if (removeIfCursor == false) {
          //remove for static function
          startPos = newValue.search("&&&");
          newValue = newValue.replace("&&&", "");
        }
        if (removeIfCursor) {
          //remove for general isnertions
    ...
    const applyViewType = async node => {
      ...
      try {
        ...
        try {
          var currentViewType = currentBlock.parentElement.parentElement.querySelector(
            ".rm-bullet"
          ).className;
          if (node["view-type"] == "bullet" && currentViewType == "rm-bullet ")
            return; //trying to set to a bullet, but already a bullet
          if (
            node["view-type"] == "document" &&
            currentViewType == "rm-bullet  opacity-none"
          )
            return; //trying to set to a document, but already a document
          if (
            node["view-type"] == "numbered" &&
            currentViewType == "rm-bullet  rm-bullet--numbered"
          )
            return; //trying to set to a number, but already a number
        } catch (e) {}
        ...
        var menuItem1 = document.querySelector(".bp3-popover-content > div> ul")
          .childNodes[9].innerText;
        var menuItem2 = document.querySelector(".bp3-popover-content > div> ul")
          .childNodes[10].innerText;
        var menuItemToClick = false;
        switch (node["view-type"]) {
          case "bullet":
            if (menuItem1 == "View as Bulleted List") menuItemToClick = 1;
            if (menuItem2 == "View as Bulleted List") menuItemToClick = 2;
            break;
          case "document":
            if (menuItem1 == "View as Document") menuItemToClick = 1;
            if (menuItem2 == "View as Document") menuItemToClick = 2;
            break;
          case "numbered":
            if (menuItem1 == "View as Numbered List") menuItemToClick = 1;
            if (menuItem2 == "View as Numbered List") menuItemToClick = 2;
            break;
        }
        if (menuItemToClick == false) await roam42KeyboardLib.pressEsc(100);
        else
          document
            .querySelector(".bp3-popover-content > div> ul")
            .childNodes[8 + menuItemToClick].childNodes[0].click();
        ...
        document.activeElement.setSelectionRange(
          document.activeElement.textLength,
          document.activeElement.textLength
        );
      } catch (e) {
    ...
    roam42.smartBlocks.outputArrayWrite = async () => {
      let countOfblocksToInsert =
        roam42.smartBlocks.activeWorkflow.arrayToWrite.length;
      if (countOfblocksToInsert > 0) {
        ...
        for (let sb of roam42.smartBlocks.activeWorkflow.arrayToWrite) {
          ...
          if (sb.reprocess == true)
            textToInsert = await roam42.smartBlocks.proccessBlockWithSmartness(
              textToInsert
            );
          if (!textToInsert.includes(roam42.smartBlocks.exclusionBlockSymbol)) {
            ...
            if (blocksInserted != countOfblocksToInsert) {
              blocksInserted += 1;
              let currentBlockId = document.querySelector(
              ...
              if (
                currentBlockId ==
                document.querySelector("textarea.rm-block-input").id
              ) {
                await roam42KeyboardLib.pressEnter(50);
              }
            }
            if (blocksInserted % 10 == 0)
              //SmartBlocks coffee break to allow Roam to catch its breath
              await roam42.common.sleep(100);
          }
        }
        //reset for next run
    ...
    roam42.smartBlocks.sbBomb = async (
      item,
      skipCursorRelocation = false,
      sbCallingSB = false,
      sbButton = false
    ) => {
      ...
      if (document.activeElement.type != "textarea") {
        roam42.common.simulateMouseClick(
          document.getElementById(roam42.smartBlocks.activeTributeTextAreaId)
        );
        await roam42.common.sleep(100);
      ...
      try {
        ...
        if (
          item.original.help &&
          roam42.smartBlocks.SmartBlockPopupHelpEnabled
        ) {
          roam42.help.displayMessage(
            '<img height="22px" src="https://cdn.glitch.com/e6cdf156-cbb9-480b-96bc-94e406043bd1%2Fgear.png?v=1605994815962">' +
              " " +
              item.original.help,
            20000
          );
        ...
        if (item.original.processor == "date") {
          skipCursorRelocation = true;
          insertSnippetIntoBlock(
            await roam42.dateProcessing
              .parseTextForDates(item.original.value)
              .trim()
          );
        }
        if (item.original.processor == "function") {
          skipCursorRelocation = true;
          await item.original.value();
        }
        if (item.original.processor == "static") {
          skipCursorRelocation = true;
          insertSnippetIntoBlock(item.original.value, false);
        }
        if (item.original.processor == "randomblock") {
          skipCursorRelocation = true;
          insertSnippetIntoBlock(
            "((" + (await roam42.common.getRandomBlock(1)) + "))"
          );
        }
        if (item.original.processor == "randompage") {
          skipCursorRelocation = true;
          insertSnippetIntoBlock(await roam42.smartBlocks.getRandomPage());
        }
        if (item.original.processor == "blocks") {
          var results = await roam42.common.getBlockInfoByUID(
            item.original.value,
            true
          );
          ...
          if (
            results[0][0].children.length == 1 &&
            !results[0][0].children[0].children
          ) {
            //has no children, just insert text into block and safe it
            skipCursorRelocation = true;
            var processedText = await roam42.smartBlocks.proccessBlockWithSmartness(
              results[0][0].children[0].string
            );
            ...
            roam42.smartBlocks.activeWorkflow.currentSmartBlockTextArea =
              document.activeElement.id;
            if (
              !processedText.includes(roam42.smartBlocks.exclusionBlockSymbol)
            )
              if (
                !processedText.includes(roam42.smartBlocks.replaceFirstBlock)
              ) {
                await insertSnippetIntoBlock(
                  processedText,
                  true,
                  removeTributeTriggerSpacer,
                  1000
                );
                await roam42.smartBlocks.processBlockAfterBlockInserted(
                  processedText
                );
            ...
            var loopStructure = async (parentNode, level) => {
              ...
              let orderedNode = await roam42.common.sortObjectsByOrder(
                parentNode
              );
              ...
              for (var i = 0; i < orderedNode.length; i++) {
                ...
                if (
                  ...
                  //used by CURRENTREFBLOCK
                  await roam42.common.sleep(
                    roam42.smartBlocks.activeWorkflow.forceDelayAferNewBlock
                  );
                  roam42.smartBlocks.activeWorkflow.forceDelayAferNewBlock = 0;
                }

                if (insertText.match(/\<\%(\s*[\S\s]*?)\%\>/))
                  //process if it has a command
                  insertText = await roam42.smartBlocks.proccessBlockWithSmartness(
                    insertText
                  );
                ...
                if (
                  !insertText.includes(roam42.smartBlocks.exclusionBlockSymbol)
                ) {
                  if (
                    firstBlock == true &&
                    ...
                    firstBlock = false;
                    insertText = await roam42.smartBlocks.processBlockAfterBlockInserted(
                      insertText
                    );
                    var txtarea = document.querySelector(
                      "textarea.rm-block-input"
                    );
                    var strPos = txtarea.selectionStart;
                    var front =
                      sbCallingSB || sbButton
                        ? txtarea.value.substring(0, strPos)
                        : txtarea.value.substring(0, strPos - 2);
                    ...
                    let txtarea = document.querySelector(
                      "textarea.rm-block-input"
                    );
                    var setValue = Object.getOwnPropertyDescriptor(
                      window.HTMLTextAreaElement.prototype,
                  ...
                  if (n.heading) {
                    ...
                    await roam42KeyboardLib.simulateKey(
                      Number(n.heading) + 48,
                      200,
                  ...
                  if (n["view-type"]) await applyViewType(n);

                  if (n["text-align"] && n["text-align"] != "left") {
                    var ev = {};
                    ev.target = document.querySelector(
                      "textarea.rm-block-input"
                    );
                    switch (n["text-align"]) {
                      case "center":
            ...
            setTimeout(() => {
              try {
                if (document.activeElement.value.includes("<%CURSOR%>")) {
                  var newValue = document.querySelector(
                    "textarea.rm-block-input"
                  ).value;
                  document.activeElement.value = "";
          ...
          if (currentSmartBlockCommand.includes("<%NOCURSOR%>")) {
            setTimeout(async () => {
              //let other commands process before exiting block edit
              // await roam42.common.sleep(500);
        ...
        roam42.smartBlocks.textBoxObserver.observe(document, {
          childList: true,
          subtree: true
        });
      } catch (e) {
    ...
    roam42.smartBlocks.buttonClickHandler = async target => {
      if (target.tagName == "BUTTON") {
        ...
        var blockInfo = (await roam42.common.getBlockInfoByUID(
          block.id.substring(block.id.length - 9)
        ))[0][0].string;
        if (blockInfo.includes(target.textContent + ":42SmartBlock:")) {
          const regex = new RegExp(
            `{{((${target.textContent})(:42SmartBlock:)(.*?))}}`
          );
          ...
          if (sbCommand == undefined) {
            ...
            roam42.help.displayMessage(
              "<b>" + params[0] + "</b> - Cannot find this #42SmartBlock",
              3000
            );
          } else {
            ...
            if (removeButton == true) {
              cursorLocation =
                blockInfo.search(commandInBlock[0]) + paddingLength;
              setValue.call(
                document.activeElement,
                blockInfo.replace(commandInBlock[0], paddingSpaces)
              );
            } else {
              cursorLocation =
                blockInfo.search(commandInBlock[0]) +
                commandInBlock[0].length +
                paddingLength;
              setValue.call(
                document.activeElement,
                blockInfo.replace(
                  commandInBlock[0],
                  commandInBlock[0] + paddingSpaces
                )
              );
            }
            var e = new Event("input", { bubbles: true });
    ...
    roam42.smartBlocks.scanForNewTextAreas = (mutationList, observer) => {
      ...
      var tribute = new Tribute({
        ...
        menuItemTemplate: function(item) {
          ...
          switch (item.original.icon) {
            case "hl":
              img =
                "https://cdn.glitch.com/e6cdf156-cbb9-480b-96bc-94e406043bd1%2Fhr.png?v=1605996193520";
              break;
            case "random":
              img =
                "https://cdn.glitch.com/e6cdf156-cbb9-480b-96bc-94e406043bd1%2Frandom.png?v=1605996193519";
              break;
            case "list":
              img =
                "https://cdn.glitch.com/463389d2-59ec-4fdc-b3c8-674037563d0e%2Flist.png?v=1606820752757";
              break;
            case "gear":
              img =
                "https://cdn.glitch.com/e6cdf156-cbb9-480b-96bc-94e406043bd1%2Fgear.png?v=1605994815962";
              break;
            case "time":
              img =
                "https://cdn.glitch.com/e6cdf156-cbb9-480b-96bc-94e406043bd1%2Fclock-time-7.png?v=1605996193660";
              break;
            default:
              img =
                "https://cdn.glitch.com/e6cdf156-cbb9-480b-96bc-94e406043bd1%2Flego3blocks.png?v=1605993127860";
          }
          return `<img width="15px" src="${img}"> ` + item.string;
        },
        menuItemLimit: 25,
      ...
      ta.addEventListener("tribute-replaced", function(e) {
        roam42.smartBlocks.tributeMenuTrigger =
          e.detail.context.mentionTriggerChar + e.detail.context.mentionText;
      });
    }; // End of scanForNewTextAreas

    roam42.smartBlocks.textBoxObserver = new MutationObserver(
      roam42.smartBlocks.scanForNewTextAreas
    );
    roam42.smartBlocks.textBoxObserver.observe(document, {
      childList: true,
      subtree: true
  ...
  window.roam42.smartBlocks.testingReload = () => {
    try {
      document.removeEventListener(
        "click",
        roam42.smartBlocks.buttonClick,
        false
      );
