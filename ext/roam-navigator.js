...
"use strict";
{
  ...
  */
  ...
  function collectNavigateOptions() {
    ...
    const {
      ...
      prefixesUsed: navigatePrefixesUsed
  ...
  function collectLinkOptions(
    ...
    const { options } = assignKeysToItems(
      linkItems,
      navigateOptions,
      navigatePrefixesUsed
    );
    ...
    if (prefix === navigateKeysPressed) {
      ...
      if (option.aliased) {
        for (const el of option.aliased) {
          ...
          // currently true.
          const extraClasses = option.extraClasses
            ? [...option.extraClasses]
            : [];
  ...
  */
  ...
  function assignKeysToItems(items, otherPrefixesUsed, otherOptions) {
    ...
    for (let q = 0; q < items.length; q++) {
      ...
      if (
        item.uid &&
        (addResult(
          uidToJumpKeys(item.uid, FILTERED_HOME_ROW_KEYS, HOME_ROW_KEYS),
          item
        ) ||
          addResult(
            uidToJumpKeys(item.uid, FILTERED_HOME_ROW_KEYS, JUMP_KEYS),
            item
          ) ||
          addResult(
            uidToJumpKeys(
              item.uid + "1",
              FILTERED_HOME_ROW_KEYS,
              HOME_ROW_KEYS
            ),
            item
          ) ||
          addResult(
            uidToJumpKeys(item.uid + "1", FILTERED_HOME_ROW_KEYS, JUMP_KEYS),
            item
          ) ||
          addResult(
            uidToJumpKeys(item.uid, FILTERED_JUMP_KEYS, JUMP_KEYS),
            item
          ))
      ) {
        /* eslint-enable */
  ...
  function navigateToElement(ev, el, f) {
    ...
    if (
      or(
        matchingClass("rm-ref-page-view-title"),
        matchingClass("rm-title-display")
      )(el)
    ) {
      withUniqueTag(
        el,
        "span",
        not(matchingClass(HINT_TYPED_CLASS)),
        clickFunc
      );
      ...
      matchingClass("bp3-icon-menu-open", "bp3-icon-menu-closed")(el)
    ) {
      click(el);
  ...
  */
  ...
  function renderBreadcrumbs(breadcrumbs) {
    withDomMutation(() => {
      ...
      const topbar = selectUnique(
        document,
        ".roam-topbar > .rm-files-dropzone >  .flex-h-box"
      );
      if (topbar) {
        const buttonClasses = ["bp3-icon-menu", "bp3-icon-menu-open"];
