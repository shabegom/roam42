...
(async () => {
  ...
  roam42.livePreview.browserHeight = await roam42.settings.get(
    "LivePreviewHeight"
  );
  roam42.livePreview.browserWidth = await roam42.settings.get(
    "LivePreviewWidth"
  );
  roam42.livePreview.delay = Number(
    await roam42.settings.get("LivePreviewDelay ")
  );

  if (roam42.livePreview.browserHeight == null)
    roam42.livePreview.browserHeight = "500px";
  if (roam42.livePreview.browserWidth == null)
    roam42.livePreview.browserWidth = "500px";
  if (roam42.livePreview.delay == null) roam42.livePreview.delay = "100";

  if (roam42.livePreview.state == "off") return;
  ...
  roam42.livePreview.livePreviewStatusToast = () => {
    ...
    iziToast.show({
      ...
      buttons: [
        ...
        [
          "<button>Enable with block refs</button>",
          function(instance, toast) {
            roam42.livePreview.setRoamLivePreviewState(2);
            instance.hide(
  ...
  if (window === window.parent) {
    ...
    const enableLivePreview = () => {
      ...
      if (window.roam42LivePreview) {
        delayTimer =
          window.roam42LivePreview.delay == undefined
            ? delayTimer
            : window.roam42LivePreview.delay;
      }
      ...
      document.addEventListener("mouseover", e => {
        ...
        if (
          e.ctrlKey == false &&
          roam42.livePreview.roam42LivePreviewState == 0
        ) {
          return;
        }
        ...
        let isPageRefNameSpace = target.classList.contains(
        ...
        if (
          ...
          isPageRef = true;
          pageIsBlock = true;
          text = target.parentElement.parentElement.getAttribute(
            "data-link-uid"
          );
        }
        ...
        if (
          isPageRef == false &&
          target.classList.contains("rm-ref-page-view-title")
        ) {
          isPageRef = true;
          ...
          setTimeout(
            ...
            delayTimer + specialDelayTimeOutAmount
          );
        }
        ...
        if (
          ...
          !isPageRefTag &&
          target.classList.length == 0 &&
          target.parentNode.classList.contains("rm-page-ref")
        ) {
          isPageRef = true;
          text = target.innerText;
          target = e.target;
        ...
        if (
          ...
          target.parentNode.classList.contains("level2")
        ) {
          isPageRef = true;
          text = target.text;
        }
        ...
        try {
          if (
            ...
            target.firstChild.classList.contains("rm-pages-title-text")
          ) {
            isPageRef = true;
        ...
        if (
          isPageRef == false &&
          (e.ctrlKey == true ||
            roam42.livePreview.roam42LivePreviewState == 2) &&
        ...
        if (isPageRef) {
          ...
          const url =
            pageIsBlock == true ? getPageUrl(text) : getPageUrlByName(text);
          ...
          if ((!isAdded(url) || !isVisible(url)) && previewIframe) {
            ...
            if (window.roam42LivePreview) {
              previewIframe.style.height =
                window.roam42LivePreview.height == undefined
                  ? "500px"
                  : window.roam42LivePreview.height;
              previewIframe.style.width =
                window.roam42LivePreview.width == undefined
                  ? "500px"
                  : window.roam42LivePreview.width;
            } else {
