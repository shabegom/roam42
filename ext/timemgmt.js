...
(() => {
  ...
  roam42.timemgmt.breadCrumbsByUID = async (
    uid,
    separator = " > ",
  ...
  roam42.timemgmt.outputTaskBlocks = async (
    tasksToProcess,
    textToProcess,
    commandMatch,
    params,
      ...
      if (outputCount <= maxToReturn - 1) {
        ...
        if (params) {
          ...
          for (let t of params) {
            ...
            } else if (!blockText.includes(tokenText)) bOutputBlock = false;
          }
        }
        if (bOutputBlock == true && outputCount <= maxToReturn - 1) {
          outputCount += 1;
          let newText = textToProcess.replace(
            commandMatch,
            `((${block.taskUID}))`
          );
          newText = await roam42.common.replaceAsync(
            newText,
            /(\<\%PAGE\%\>)/g,
            async (match, name) => {
          ...
          newText = await roam42.common.replaceAsync(
            ...
            /(\<\%PATH:)(\s*[\S\s]*?)(\%\>)/g,
            async (match, name) => {
              let commandToProcess = match
                .replace("<%PATH:", "")
                .replace("%>", "");
              let vValue = roam42.timemgmt.breadCrumbsByUID(
                block.taskUID,
                commandToProcess
              );
              return vValue;
          ...
          newText = await roam42.smartBlocks.proccessBlockWithSmartness(
            newText
          );
    ...
    // return '';
    if (outputCount >= 1) return roam42.smartBlocks.replaceFirstBlock;
  ...
  var parseTodoRequestString = requestString => {
    ...
    if (requestString.includes(",")) {
      limitOutputCount = Number(
        requestString.substring(0, requestString.search(","))
      );
      queryParameters = requestString
        .substring(requestString.search(",") + 1, requestString.length)
        .split(",");
    } else limitOutputCount = requestString;
    ...
    for (var task of await roam42.timemgmt.getAllTasks()) {
      try {
        ...
        if (taskString.substring(0, 12) != "{{[[query]]:") {
          if (
            outputCounter < limitOutputCount &&
            taskString.includes("{{[[TODO]]}}") &&
            taskString.includes(todayDate)
          ) {
            outputCounter += 1;
            outputTODOs.push({
              taskUID: task[0].uid,
              taskString: task[0].string,
              pageTitle: task[1].title
            });
          }
        }
      } catch (e) {}
  ...
  roam42.timemgmt.smartBlocks.commands.todosDueToday = async (
    requestString,
    textToProcess,
    commandMatch
  ) => {
    ...
    return roam42.timemgmt.outputTaskBlocks(
      results,
      textToProcess,
      commandMatch,
      request.params,
      ...
      if (
        confirm(
          "Would you like to insert " +
            ...
            " blocks refs with Overdue TODOS?"
        )
      )
        await roam42.smartBlocks.outputArrayWrite();
    } else await roam42.smartBlocks.outputArrayWrite();
  };

  // OVERDUE COMMAND to use in workflow
  roam42.timemgmt.smartBlocks.commands.todosOverdue = async (
    requestString,
    includeDNP = false,
    textToProcess,
    commandMatch
    ...
    if (roam42.smartBlocks.activeWorkflow.arrayToWrite.length > 10) {
      if (
        confirm(
          "Would you like to insert " +
            roam42.smartBlocks.activeWorkflow.arrayToWrite.length +
            " blocks refs with Overdue+DNP TODOS?"
        )
      )
        await roam42.smartBlocks.outputArrayWrite();
    } else await roam42.smartBlocks.outputArrayWrite();
  ...
  roam42.timemgmt.smartBlocks.commands.todosFuture = async (
    requestString,
    includeDNP = false,
    textToProcess,
    commandMatch
    ...
    if (roam42.smartBlocks.activeWorkflow.arrayToWrite.length > 10) {
      if (
        confirm(
          "Would you like to insert " +
            roam42.smartBlocks.activeWorkflow.arrayToWrite.length +
            " blocks refs with undated TODOS?"
        )
      )
        await roam42.smartBlocks.outputArrayWrite();
    } else await roam42.smartBlocks.outputArrayWrite();
  ...
  roam42.timemgmt.smartBlocks.commands.todoNotDated = async (
    requestString = 50,
    textToProcess,
    commandMatch
  ) => {
  ...
  roam42.timemgmt.todosOverdue = async (
    limitOutputCount = 50,
    sortAscending = true,
    includeDNPTasks = true
  ) => {
    var yesterday = roam42.dateProcessing.testIfRoamDateAndConvert(
      roam42.dateProcessing.parseTextForDates("yesterday")
    );
    var outputTODOs = [];
    var outputCounter = 0;
    ...
    for (var task of await roam42.timemgmt.getAllTasks()) {
      if (outputCounter < limitOutputCount) {
        ...
        var taskString = task[0].string + " ";
        if (taskString.substring(0, 12) != "{{[[query]]:") {
          if (taskString.includes("{{[[TODO]]}}"))
            //confirm there is a TODO in the string properly formatted
            testForPages = taskString
              .replace("[[TODO]]", "")
              .match(/\[\[(\s*[\S\s]*?)\]\]/g);
          if (testForPages != null) {
            for (let page of testForPages) {
              try {
                var testForDate = roam42.dateProcessing.testIfRoamDateAndConvert(
                  page
                );
        ...
        if (includeDNPTasks && taskWasOutputted == false) {
          try {
            var pageNameIsDate = roam42.dateProcessing.testIfRoamDateAndConvert(
              task[1].title
            );
    ...
    return outputTODOs
      .sort((a, b) => a.pageTitle - b.pageTitle)
      .sort((a, b) => (sortAscending ? a.date - b.date : b.date - a.date));
  };

  roam42.timemgmt.todosFuture = async (
    limitOutputCount = 50,
    sortAscending = true,
    includeDNPTasks = true
  ) => {
    var tomorrow = roam42.dateProcessing.testIfRoamDateAndConvert(
      roam42.dateProcessing.parseTextForDates("tomorrow")
    );
    var outputTODOs = [];
    var outputCounter = 0;
    //STEPS: (1) loop through each tag to see if it is a date before today (2) Also check if page name is dated
        ...
        if (testForPages) {
          for (let page of testForPages) {
            try {
              ...
              if (testForDate) todoContainsDate = true;
            } catch (e) {}
          }
        } // end of first IF
...
(() => {
  ...
  var tagQuery = async pageRef => {
    var results = [];
    for (var block of await roam42.common.getBlocksReferringToThisPage(
      pageRef
    )) {
      try {
      ...
      for (var block of await tagQuery(firstPage)) {
        if (outputCounter < limitOutputCount) {
          ...
          for (var t of tokens) {
            ...
            } else if (!blockText.includes(tokenText)) bIncludeRef = false;
          }
          if (bIncludeRef == true) {
            results.push(block);
            outputCounter += 1;
  ...
  roam42.q.smartBlocks.blockMentions = async () => {
    ...
    if (
      query.length > 20 &&
      confirm(
        `There are ${query.length +
          1} blocks matching your request. Continue with inserting blocks?`
      ) == false
    )
      return;
  ...
  roam42.q.smartBlocks.commands.blockMentions = async (
    requestString,
    textToProcess
  ) => {
    var limitOutputCount = Number(
      requestString.substring(0, requestString.search(","))
    );
    ...
    var queryParameters = requestString.substring(
      requestString.search(",") + 1,
      requestString.length
    );
    var UIDS = [];
    for (var block of await roam42.q.blockMentions(
      queryParameters,
      limitOutputCount + 1
    ))
      UIDS.push(block.uid);
    ...
    for (var block of results) {
      var newText = await roam42.common.replaceAsync(
        textToProcess,
      ...
      newText = await roam42.common.replaceAsync(
        ...
        async (match, name) => {
          ...
          var vValue = roam42.timemgmt.breadCrumbsByUID(
            block[0].uid,
            commandToProcess
          );
          return vValue;
    ...
    if (results.length > 0) return roam42.smartBlocks.replaceFirstBlock;
    else return roam42.smartBlocks.exclusionBlockSymbol;
  };
  ...
  //  5 - Sort order
  //  6 - Filter parameters
  roam42.q.smartBlocks.commands.blockMentionsDated = async (
    requestString,
    textToProcess
  ) => {
    ...
    if (params.length < 4)
      return "BLOCKMENTIONSDATED requires atleast 4 parameters";
    ...
    var startDate = roam42.dateProcessing
      .parseTextForDates(params.shift())
      .replace("[[", "")
      .replace("]]", "");
    var endDate = roam42.dateProcessing
      .parseTextForDates(params.shift())
      .replace("[[", "")
      .replace("]]", "");

    if (startDate == "-1" && endDate == "-1") bReturnUndatedTodos = true;
    else {
      startDate =
        startDate != 0
          ? await Date.parse(chrono.parseDate(startDate))
          : Date.parse("1-01-01");
      endDate =
        endDate != 0
          ? await Date.parse(chrono.parseDate(endDate))
          : Date.parse("9999-012-30");
    }
          ...
          if (pageRefs != null) {
            for (let ref of pageRefs) {
              try {
                ...
                if (bReturnUndatedTodos == false) {
                  //skip this block, it has a date
                  if (
                    testForDate &&
                    startDate <= testForDate &&
                    endDate >= testForDate
                  )
                    outputThisBlock = true; //has a date, and dates should be outut
                } else if (!testForDate) outputThisBlock = true; //if not a date, write out (user requested not dated)
              } catch (e) {}
            }
          } //no page refs at all
          else if (bReturnUndatedTodos) outputThisBlock = true;
        }
        if (outputThisBlock) {
          outputCounter += 1;
  ...
  roam42.q.smartBlocks.commands.search = async (
    requestString,
    textToProcess
  ) => {
    var limitOutputCount =
      Number(requestString.substring(0, requestString.search(","))) + 1;
    var queryParameters = requestString.substring(
      requestString.search(",") + 1,
      requestString.length
    );
    var outputCounter = 0;
    var UIDS = [];
    for (var block of await roam42.q.search(
      queryParameters,
      limitOutputCount
    )) {
      if (outputCounter < limitOutputCount) UIDS.push(block[0].uid);
      outputCounter++;
    }
    var results = await roam42.common.getPageNamesFromBlockUidList(UIDS);
    for (var block of results) {
