  ...
  roam42.jumpnav.jumpCommand = (event, handler) => {
    ...
    if (["ctrl+j t", "ctrl+j b"].includes(handler)) {
      ...
      handler == "ctrl+j b" || handler == "˚"
        ...
        : roam42.common.simulateMouseClick(
            articleContent[0].querySelector(".rm-block-text")
          );
      setTimeout(() => {
    ...
    if (
      ["ctrl+j j", "ctrl+j k", "ctrl+j i", "ctrl+j u", "ctrl+j d"].includes(
        handler
      )
    ) {
      switch (handler) {
        ...
        switch (handler) {
          case "ctrl+j s": // copy block ref as alias
            setTimeout(async () => {
              ...
              var formatToUse = await roam42.settings.get(
                "CopyRefAsAliasFormat"
              );
              var outputText = "";
          ...
          case "ctrl+j r": // copy block ref
            ...
            roam42.help.displayMessage(
              `<b>Roam<sup>42</sup></b><br/>Copied: ((${uid}))`,
              2000
            );
            break;
        }
    ...
    if (
      ...
      var locFacts = roam42.common.saveLocationParametersOfTextArea(
        event.target
      );
      var parentControlNode = "";
      if (
        document.getElementById(locFacts.id).parentNode.parentNode.tagName ==
        "DIV"
      ) {
      ...
      roam42.common.simulateMouseClickRight(
        parentControlNode.previousSibling.childNodes[1]
      );
      setTimeout(() => {
        switch (handler) {
    ...
    if (["ctrl+j 5", "ctrl+j 6", "ctrl+j 7"].includes(handler)) {
      switch (handler) {
        case "ctrl+j 5": // heading 1
          roam42KeyboardLib.simulateKey(49, 200, {
            ctrlKey: true,
            altKey: true
          }); //49 is key 1
          roam42KeyboardLib.simulateKey(49, 200, {
        ...
        case "ctrl+j 6": // heading 2
          roam42KeyboardLib.simulateKey(50, 200, {
            ctrlKey: true,
            altKey: true
          }); //49 is key 1
          roam42KeyboardLib.simulateKey(50, 200, {
        ...
        case "ctrl+j 7": // heading 3
          roam42KeyboardLib.simulateKey(51, 200, {
            ctrlKey: true,
            altKey: true
          }); //49 is key 1
          roam42KeyboardLib.simulateKey(51, 200, {
    ...
    if (["ctrl+j e", "ctrl+j c", "ctrl+j o"].includes(handler)) {
      var locFacts =
        event.srcElement.localName == "textarea"
          ? roam42.common.saveLocationParametersOfTextArea(event.target)
          : "";
      ...
      setTimeout(() => {
        switch (handler) {
          case "ctrl+j e":
            document
              .querySelector(".bp3-popover-content > div> ul")
              .childNodes[2 + zoomedView].childNodes[0].click();
            break;
          case "ctrl+j c":
            document
              .querySelector(".bp3-popover-content > div> ul")
              .childNodes[3 + zoomedView].childNodes[0].click();
            break;
          case "ctrl+j o":
            document
              .querySelector(".bp3-popover-content > div> ul")
              .childNodes[1 + zoomedView].childNodes[0].click();
            break;
        }
        if (locFacts != "") {
          setTimeout(() => {
