...
(() => {
  ...
  roam42.jumpToDate.component = {
    ...
    moveForwardToDate(bForward) {
      let jumpDate = chrono.parseDate(
        document.querySelector(".rm-title-display").innerText
      );
      let directionTip = "";
      let calIcon = "";
      if (jumpDate != null) {
    ...
    async navigateUIToDate(destinationDate, useShiftKey) {
      ...
      setTimeout(() => {
        if (
          roam42.keyevents.shiftKeyDownTracker == true &&
          useShiftKey == true
        ) {
    ...
    initialize() {
      ...
      try {
        ...
        console.log(
          "could not add toolbar buton - see module jump-to-date.js "
        );
        console.log(e);
      }
      ...
      this.rqrJumpToDatePanel.options.onbeforeclose.push(function() {
        document.querySelector("#rqrJumpToDatePanel").style.visibility =
          "hidden";
        document.querySelector("#jumptoDateInput").style.visibility = "hidden";
        return false;
      });

      flatpickr("#jumptoDateInput", {
        dateFormat: "Y-m-d",
        weekNumbers: true,
      ...
      this.flCalendar.config.onValueUpdate.push(
      ...
      this.flCalendar.config.onClose.push(
        (selectedDates, dateStr, instance) => {
